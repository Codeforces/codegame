...
pub type Generator = trans_gen::gens::java::Generator;
...
impl<G: Game> ClientGen<G> for trans_gen::GeneratorImpl<Generator> {
    ...
    fn gen(options: &Options, gen_options: Self::GenOptions) -> anyhow::Result<()> {
        ...
        write_file(
            ...
            &project_file!(options, "MyStrategy.java"),
        )?;
        write_file(
            src_path.join("DebugInterface.java"),
            &project_file!(options, "DebugInterface.java"),
        )?;
        write_file(
            src_path.join("Runner.java"),
            &project_file!(options, "Runner.java"),
        )?;
        write_file(
